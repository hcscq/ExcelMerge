...
namespace Server
{
    class Merge
    {
        private const string Str_MainExcelPath = ".\\Main.xlsx";
        //private string Str_CurChildExcelPath = "";
        //private string Str_ChildrenDir =  "\\Children";
        ...
        public void MergeExcel(List<string> ChildrenFilesName,string destMain)
        {
            if (!File.Exists(Str_MainExcelPath))
            {
                Console.WriteLine("在此程序当前目录下放主Excel,名字为：Main.xlsx（注意是XLSX）！");
                return;
            }
            File.Copy(Str_MainExcelPath, destMain);

            Con_MainExcel = ExcelHelper.CreateConnection(destMain, ExcelHelper.ExcelVerion.Excel2007);
            ...
            for (int m = 0; m < ChildrenFilesName.Count; m++)
            {
                ...
                try
                {
                    ...
                    foreach (var it in ChildSheetName)
                    {
                        ...
                        if (it.Contains("_"))
                        {
                            Console.WriteLine("检查文件" + fn + "\r\n是否启用了筛选功能.目前将跳过该文件的工作簿:" + it + ".");
                        ...
                        catch (Exception e1)
                        {
                            ...
                            retry = false;
                            Console.WriteLine("文件" + fn + "处理出现问题.\r\n工作表名：" + it + e1.Message);
                            continue;
                        }
                ...
                catch (Exception e1)
                {
                    ...
                    else
                    {
                        ...
                        Logger.WriteLog(loc_info.ToString() + e1.Message + "\r\n");
                        retry = false;
                        Console.WriteLine("文件" + fn + "处理出现问题.截图发给..\r\n" + e1.Message);
                    }
                }
            }
